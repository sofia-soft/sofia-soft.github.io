...
document.addEventListener('DOMContentLoaded', () => {
    ...
    (function initHero() {
        ...
        if (hasWordSpans) {
            ...
            const hwStyle = 'display:inline-block;opacity:0;transform:translateY(22px) rotate(-1deg);will-change:transform,opacity';
            const spans   = [];
            ...
            spans.forEach((sp, i) => {
                setTimeout(() => {
                    sp.style.transition = `opacity .75s ${EASE}, transform .8s ${EASE}`;
                    sp.style.opacity    = '1';
                    sp.style.transform  = 'translateY(0) rotate(0deg)';
                }, 430 + i * 80);
            });
        }
    ...
    function initStagger(containerSel, cardSel, opts = {}) {
        ...
        const { delay = 160, duration = 0.85, offsetY = 38, scale = 0.93, threshold = 0.12, rootMargin = '0px 0px -70px 0px' } = opts;

        const st = document.createElement('style');
        st.textContent = `${cardSel} { opacity:0; will-change:opacity,transform; }`;
        document.head.appendChild(st);

        cards.forEach(card => { card.style.transform = `translateY(${offsetY}px) scale(${scale})`; });
    ...
    if (aboutSection) {
        ...
        aboutSection.querySelectorAll('.about_us_title_content p').forEach(p => { p.style.transform = 'translateY(16px)'; });
        aboutSection.querySelectorAll('.about_us_list li').forEach(li => { li.style.transform = 'translateX(-14px)'; });
    ...
    if (contactSection) {
        ...
        contactSection.querySelectorAll('.contact_list li').forEach(li => { li.style.transform = 'translateX(28px)'; });
